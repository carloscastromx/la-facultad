<?php
    ...
    $consulta = $mysqli->query("SELECT pedidos.id_pedido, nombre, DATE_FORMAT(fecha_hora, '%d/%m/%Y %H:%i') as fecha_pedido, sum(subtotal) as total_pedido from clientes INNER JOIN pedidos ON pedidos.id_cliente = clientes.id_cliente INNER JOIN detalle_pedidos on detalle_pedidos.id_pedido = pedidos.id_pedido WHERE id_sucursal = $id_sucursal AND (DATE_FORMAT(fecha_hora, '%Y-%m-%d') >= '$fecha_old') AND (DATE_FORMAT(fecha_hora, '%Y-%m-%d') <='$fecha_act') GROUP BY pedidos.id_pedido, nombre, fecha_pedido ORDER BY fecha_hora DESC");
...
<body>
    ...
    <section>
        ...
        <div class="tabla-resumen-ordenes">
            <div class="fila-resumen-ordenes f-encabezados">
                <p>ID Pedido</p>
                <p>Cliente</p>
                <p>Fecha/Hora</p>
                <p>Total</p>
            ...
            <?php foreach($consulta as $o){ ?>
                <div class="fila-datos-resumen-ordenes">
                    <p><?= $o['id_pedido'] ?></p>
                    <p><?= $o['nombre']; ?></p>
                    <p><?= $o['fecha_pedido']; ?></p>
                    <p>$<?= number_format($o['total_pedido'],2,".",","); ?></p>
                </div>
            <?php } ?>
        </div>
    </section>
